...
public class Main {
    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            int[] dp = new int[maxn];
            Arrays.fill(dp,1);
            IntStream.range(1, maxn).forEachOrdered(i -> {
                dp[i] = dp[i - 1];
                dp[i] = dp[i] << 1;
                dp[i] %= MOD;
            });
            ...
            while (testcases-- > 0) {
                ...
                int[] a = new int[n];
                int[] b = new int[n];
                for (int i=0;i<n;i++) a[i] = r.ni()-1;
                for (int i=0;i<n;i++) b[i] = r.ni()-1;

                g = new ArrayList<>();
                for (int i=0;i<n;i++) g.add(new ArrayList<>());

                for (int i=0;i<n;i++) addEdge(g,a[i],b[i]);
