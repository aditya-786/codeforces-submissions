    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                for (int i=0;i<n;i++) arr.add(r.nl());

                if(n==1){
                    out.write((arr.get(0) + " ").getBytes());
                }else {
                    long ans = 0;
                    for (int i = 0; i < n; i++) {
                        ans += i == 0 ? arr.get(i) : Math.abs(arr.get(i) - arr.get(i - 1));
                    }
                    ans += arr.get(n - 1);
                    for (int i = 0; i < n; i++) {
                        if (i != 0) {
                            if (i < n - 1) ans -= Math.max(0L, arr.get(i) - Math.max(arr.get(i - 1), arr.get(i + 1)));
                            else ans -= Math.max(0L, arr.get(i) - arr.get(i - 1));
                        } else ans -= Math.max(0L, arr.get(i) - arr.get(i + 1));
                    }
