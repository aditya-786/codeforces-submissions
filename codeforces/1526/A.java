    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                int n = r.ni();
                int[] arr = new int[n*2];
                for (int i=0;i<n*2;i++) arr[i] = r.ni();

                int[] temp = new int[n*2];
                for (int i=0;i<n*2;i++) temp[i] = arr[i];
                Arrays.sort(temp);
                    int k = 0;
                    for (int i=1;i<n*2;i+=2,k++) arr[i] = temp[k];
                    k = n*2-1;

                    for (int i=0;i<n*2;i+=2,k--) arr[i] = temp[k];

                    for (int ele : arr) out.write((ele + " ").getBytes());
                    out.write(("\n").getBytes());
                }
            // Solution Ends Here
        } catch (IOException e) {
