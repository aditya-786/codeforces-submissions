    ...
    static void run(){
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases --> 0){
                ...
                for (int i=0;i<n;i++) time.add(r.ni());

                long ans1 = 0, ans2 = 0;

                int i=1;
                while (i<=n) {
                    int t = Math.abs(arrival.get(i) - departure.get(i-1));
                    ans1 = ans2 + (t+time.get(i-1));
                    int w = Math.abs(arrival.get(i) - departure.get(i));
                    w = (int) Math.ceil(w/2.0);
                    ans2 = ans1 + w;
                    ans2 = Math.max(ans2, departure.get(i));
                    i++;
                }
