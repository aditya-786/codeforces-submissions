...
public class Main {
    ...
    static void CycleDetection(ArrayList<ArrayList<Integer>>graph, int vertices) {
        boolean[] visited = new boolean[vertices];
        for (int i = 1; i < vertices; i++) {
            if (!visited[i]) {
                if (DFS(graph, -1, i, visited)) count++;
            }
        }
    }
