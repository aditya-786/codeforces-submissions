...
public class Main {
    ...
    static long lower(int low,int high,List<Long> arr,long x)
    {
        while (low <= high) {
            int mid = (low + high) / 2;
            if (arr.get(mid) >= x)
    ...
    static long upper(int low,int high,List<Long> arr,long y)
    {
        while (low <= high) {
            int mid = (low + high) / 2;
            if (arr.get(mid) <= y)
    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                long count = 0;
                if (R<L) out.write((0 + " ").getBytes());
                else {

                    for (int i=0;i<n-1;i++){
                        count+=(upper(i+1,n-1,list,R-list.get(i))-lower(i+1,n-1,list,L-list.get(i))+1);
                    }
