    ...
    static void run() {
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0) {
                ...
                int count = 0;
                int ind = 0;
                for(int i=0;i<n;i++) if ((arr[i] & 1) == 0) temp[ind++] = arr[i];

                for(int i=0;i<n;i++) if ((arr[i] & 1) == 1) temp[ind++] = arr[i];

                for(int i=0;i<ind;i++) for (int j = i + 1; j < ind; j++) if (gcd(temp[i], 2L * temp[j]) > 1) count++;
