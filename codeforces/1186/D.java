...
public class Main {
    ...
    static int[][] dp = new int[300012][2];
   // static List<Integer> list = new ArrayList<>();
    ...
    static int find(List<Integer> list)
    {
        ...
        for (int i=1;i<=size;i++)
        {
            ...
            dp[i][0] = Math.max(dp[i-1][0], dp[i-1][1]+ele);
            dp[i][1] = Math.max(dp[i-1][1], dp[i-1][0]-ele);
        }

        return dp[size][0];
    }

    static int find(int x, int y, List<Integer> al)
    {
        List<Integer> list = new ArrayList<>();
        for (int i=x-1;i<=y+1;i++) list.add(al.get(i));
    ...
    public static void main(String[] args) throws java.lang.Exception
    {
        try
        {
            ...
            try (OutputStream out = new BufferedOutputStream(System.out))
            {
                ...
                for (int i=1;i<=n;i++)
                {
                    double ele = fr.nextDouble();
                    list1.add(ele);

                    list2.add((int) Math.floor(ele));

                    sum+=list2.get(i);

                    list3.add((int) Math.ceil(ele));
                }

                long absAns = Math.abs(sum);

                for (int i=1;i<=n;i++)
                {
                    res.add(list2.get(i));

                    if (absAns >0 && list3.get(i)>list2.get(i))
                    {
                        absAns--;
                        res.set(i,list3.get(i));
                    }
                }

                for (int i=1;i<=n;i++) out.write((res.get(i)+"\n").getBytes());
