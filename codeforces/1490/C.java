...
public class Main {
    ...
    static boolean sumSquare(long n)
    {
        BigInteger bi = new BigInteger(String.valueOf(n));
        HashMap<BigInteger, Integer> s = new HashMap<>();
        //for (long i = 0; i * i*i <= n; ++i)
        for (BigInteger i = BigInteger.valueOf(1);i.multiply(i).multiply(i).compareTo(BigInteger.valueOf(n))<0;i = i.add(BigInteger.ONE))
        {

            // store square value in hashmap
            BigInteger temp = (i.multiply(i).multiply(i));
            s.put(i.multiply(i).multiply(i), 1);
            if (s.containsKey(bi.subtract(temp)))
            {
                return true;
            }
        }
...
//                }
                ...
                if (sumSquare(n) && n>1) out.write(("YES" + " ").getBytes());
