...
public class Main {
    ...
    // for global initializations and methods
    ...
    static void run(){
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            start : while (testcases --> 0) {
                ...
                ArrayList<ArrayList<Integer>> list = IntStream.range(0, n).<ArrayList<Integer>>mapToObj(i -> new ArrayList<>()).collect(Collectors.toCollection(ArrayList::new));

                {
                    int i = 0;
                    while (i < n) {
                        for (int j = 0; j < m; j++) list.get(i).add(r.ni());
                        i++;
                    }
                }
                ...
                List<Pair> pairs = new ArrayList<>();
                {
                    int i = 0;
                    while (i < n) {
                        for (int j = 0; j < m; j++) pairs.add(new Pair(list.get(i).get(j), i));
                        i++;
                    }
                }
...

                {
                    int i = 0, j = 0;
                    while (i < m) {
                        int second = pairs.get(j).second;
                        int first = pairs.get(j).first;
                        int parIndex = IntStream.range(0, m).filter(k -> list.get(second).get(k) == first && !visited[second][k]).findFirst().orElse(0);

                        int val1 = list.get(second).get(parIndex);
                        int val2 = list.get(second).get(i);

                        list.get(second).set(parIndex, val2);
                        list.get(second).set(i, val1);

                        visited[second][i] = true;
                        i++;
                        j++;
                    }
                }
