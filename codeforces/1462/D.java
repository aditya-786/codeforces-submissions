        ...
        try {
            ...
            try (OutputStream out = new BufferedOutputStream(System.out)) {
                ...
                while (testcases-- > 0) {
                    ...
                    long sum = 0;
                    int ind = 0;
                    for (int i=1;i<=n;i++) sum+=list.get(i);
                    ...
                    for (int i=0;i<=n-1;i++){
                        ...
                        al.add(0);
                        for (int j=1;j<=n;j++) al.add(list.get(j));
                        al.add(0);

                        boolean ff = true;

                        for (int j=1;j<=n;j++){
                            if (al.get(j) == ele) continue;
                            else if (al.get(j)>ele) ff = false;
                            else if (al.get(j)<ele){
                                rr++;
                                al.set(j+1, al.get(j+1)+al.get(j));
                            }
                        }
