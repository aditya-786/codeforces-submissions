...
public class Main {
    ...
    try (OutputStream out = new BufferedOutputStream(System.out)) {
        ...
        start : while (testcases --> 0) {
            ...
            Arrays.sort(temp);
            if (arr[0] == temp[0] && arr[n - 1] == temp[n - 1]) {
                boolean ff = false;
                for (int i = 1; i < n - 1; i++) {
                    if (arr[i] != temp[i]) {
                        ff = true;
                ...
                if (ff) out.write((1 + " ").getBytes());
                else out.write((0 + " ").getBytes());
            } else {
                if (arr[0] == temp[0]) {
                    out.write((1 + " ").getBytes());
                } else if (arr[n - 1] == temp[n - 1]) out.write((1 + " ").getBytes());
                else {
                    if (arr[0] == temp[n - 1] && arr[n - 1] == temp[0]) out.write((3 + " ").getBytes());
