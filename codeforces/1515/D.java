...
public class Main {
    ...
    static boolean checkProperSquare(int n){
    ...
    static void run(){
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            start : while (testcases --> 0) {
                ...
                IntStream.range(0, left).map(i -> r.ni()).forEachOrdered(ele -> map.put(ele, map.getOrDefault(ele, 0) + 1));
                IntStream.range(0, right).map(i -> r.ni()).forEachOrdered(ele -> map.put(ele, map.getOrDefault(ele, 0) - 1));
                ...
                if (left < right) {
                    int diff = right-left;
                    int minDiff = Math.min(diff/2, temp1);
                    finalAns+=diff-minDiff;
                } else {
                    int diff = left-right;
                    int minDiff = Math.min(diff/2, temp2);
                    finalAns+=diff-minDiff;
                }

                finalAns+=Math.min(leftAns, rightAns);

                out.write((finalAns + " ").getBytes());
                out.write(("\n").getBytes());
