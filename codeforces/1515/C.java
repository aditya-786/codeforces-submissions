    ...
    static void run(){
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            start : while (testcases --> 0) {
                ...
                PriorityQueue<Pair> priorityQueue = new PriorityQueue<>(Comparator.comparingInt(a -> a.second));
                int[] ans = new int[n];
                IntStream.range(0, m).mapToObj(i -> new Pair(i + 1, 0)).forEachOrdered(priorityQueue::add);
                ...
                // ans cant be negative... its always possible
                out.write(("YES" + " ").getBytes());
                out.write(("\n").getBytes());
                for (int i : ans) out.write((i + " ").getBytes());
                out.write(("\n").getBytes());
