...
public class Main {
    ...
    try (OutputStream out = new BufferedOutputStream(System.out)) {
        ...
        start : while (testcases --> 0) {
            ...
            int n = fr.ni();
            StringBuilder sb = new StringBuilder(fr.word());

            Map<Character, Integer> map = new HashMap<>();
            for (char c : sb.toString().toCharArray()) map.put(c,map.getOrDefault(c,0)+1);
            ...
            while (i<n-1) {
                if (sb.charAt(i) == sb.charAt(i+1)) count++;
                    ...
                    if (map.getOrDefault(sb.charAt(i),0)!=count) ff = true;
                    count = 1;
                }
                i++;
            }

            if (map.getOrDefault(sb.charAt(n-1),0)!=count) ff = true;
            out.write(ff ? ("NO" + " ").getBytes() : ("YES" + " ").getBytes());
            out.write(("\n").getBytes());
