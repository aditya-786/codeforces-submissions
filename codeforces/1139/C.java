...
public class Main
{
    ...
    static void DFS(ArrayList<ArrayList<Integer>> graph, int source, boolean[] visited)
    {
        visited[source] = true;

        count++;

        for (int v : graph.get(source)) if (!visited[v]) DFS(graph, v, visited);
    }
        ...
        try
        {
            ...
            try (OutputStream out = new BufferedOutputStream(System.out))
            {
                ...
                while (testcases -- > 0)
                {
                    ...
                    for (int i=0;i<n-1;i++)
                    {
                        int v1 = r.ni(), v2 = r.ni(), color = r.ni();

                        if (color!=1) addEdge(graph, v1, v2);
                    }

                    int ans = (int)powerStrings(String.valueOf(n), String.valueOf(k));
                    ...
                    for (int i=1;i<=n;i++)
                    {
                        if (!visited[i])
                        {
                            ...
                            ans = (int) ((((ans-powerStrings(String.valueOf(count),String.valueOf(k)))%MOD)+MOD)%MOD);
                        }
                    }

                    out.write((ans%MOD+"").getBytes());
                }
            }
        }
        catch(Exception e){}
