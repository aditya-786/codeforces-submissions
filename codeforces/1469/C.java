    ...
    static void run(){
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            while (testcases-- > 0){
                ...
                long startingVar = list.get(0);
                long LastVar = list.get(0)+k-1;
                boolean ff = true;
                for (int i=1;i<n-1;i++){
                    if (list.get(i) > LastVar || list.get(i) + (k << 1) - 2 < startingVar) {
                        ff = false;
                        break;
                    }

                    LastVar = list.get(i) + k - 1 >= LastVar ? LastVar + k - 1 : list.get(i) + k - 1 + k - 1;

                    startingVar= list.get(i) + k - 1 >= startingVar ? list.get(i) : list.get(i) + startingVar - list.get(i) - (k - 1);
                }

                if (list.get(n-1)>LastVar || list.get(n-1)+k-1<startingVar) ff = false;

                out.write(ff ? ("Yes" + " ").getBytes() : ("No" + " ").getBytes());
                out.write(("\n").getBytes());
            }
        } catch (IOException e) {
            e.printStackTrace();
