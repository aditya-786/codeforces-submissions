...
public class Main {
    ...
    public static int pos2(int[] arr, int val){
    ...
    public static int pos1 (int[] arr, int val){
        int left = 0,right = arr.length-1,index = -1;
        if (left <= right) {
            do {
                int mid = left + (right - left) / 2;
        ...
        try (OutputStream out = new BufferedOutputStream(System.out)) {
            ...
            start : while (testcases --> 0) {
...

                {
                    int i=1;
                    while (i<=2*n) {
                        if(!set.contains(i)) dp1 [counter ++]=i;
                        i++;
                    }
                }
                counter =0;
                boolean[] countLeft = new boolean[n];
                boolean[] countRight = new boolean[n];

                {
                    int i=0;
                    while (i<n) {
                        int index = pos1 (dp1 ,arr[i]);
                        if (index == -1) {
                            break;
                        } else {
                            countLeft [i]=true;
                            counter =Math.max(counter +1,index +1);
                            if(counter ==n) break;
                        }
                        i++;
                    }
...

                {
                    int i=n-1;
                    while (i>=0) {
                        int index = pos2(dp1 , arr[i]);
                        if (index == -1) {
                            break;
                        } else {
                            countRight [i] = true;
                            counter  = Math.min(index - 1, counter  - 1);
                            if (counter  < 0) break;
                        }
                        i--;
                    }
                ...
                int ans=0;
                if(countRight [0]) ans++;
                if(countLeft [n-1]) ans++;
                ans += IntStream.range(0, n - 1).filter(i -> countLeft[i] && countRight[i + 1]).count();
