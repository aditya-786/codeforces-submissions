        ...
        try
        {
            ...
            try (OutputStream out = new BufferedOutputStream(System.out))
            {
                ...
                while (testcases -- > 0)
                {
                    ...
                    long minPow = Math.min((long)(Math.pow(2,(long)(Math.log(h)/Math.log(2)))),(long) (Math.pow(2,(long)(Math.log(w)/Math.log(2)))));

                    long maxHeight = (long) Math.min(h, 1.25*minPow);
                    long maxWidth = (long) Math.min(w, 1.25*minPow);

                    String s = maxHeight>=maxWidth ? maxHeight+" "+minPow : minPow+" "+maxWidth;
