...
//                       arr[pos-1]++;
                    ...
                    Arrays.fill(count,1);
                    for (int i=1;i<n+1;i++) count[i]=count[i-1]+arr[i];

                   // for (int i=1;i<=25;i++) arr[i-1]+=arr[i];

                    int[] finalCount = new int[26];
                    int k = 1;
                    for (char c : s.toCharArray()) finalCount[c-'a']+=count[k++];

                    for (int i=0;i<26;i++) out.write((finalCount[i] + " ").getBytes());
                    out.write(("\n").getBytes());
                }
            }
        }
